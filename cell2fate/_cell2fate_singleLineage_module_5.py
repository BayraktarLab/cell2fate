...
class DifferentiationModel_OneLineage_DiscreteTwoStateTranscriptionRate(PyroModule):
    ...
    def __init__(
        ...
        activation_rate_hyp_prior={"mean_hyp_prior_mean": 1., "mean_hyp_prior_sd": 0.01,
                                    "sd_hyp_prior_mean": 0.02, "sd_hyp_prior_sd": 0.01},
    ...
    def forward(self, u_data, s_data, idx, batch_index):
        ...
        alpha_ONkg = pyro.sample('alpha_ONkg', dist.Gamma(G_a(alpha_mu, alpha_sd), G_b(alpha_mu, alpha_sd)).expand([self.n_dim, 1, self.n_vars, 1]).to_event(4))       
        ...
        beta_kg = pyro.sample('beta_kg', dist.Gamma(G_a(beta_mu, beta_sd), G_b(beta_mu, beta_sd)).expand([self.n_dim, 1,self.n_vars,1]).to_event(3))
        # Degredation rate:
        ...
        gamma_kg = pyro.sample('gamma_kg', dist.Gamma(G_a(gamma_mu, gamma_sd), G_b(gamma_mu, gamma_sd)).expand([self.n_dim, 1, self.n_vars,1]).to_event(3))
        ...
        lam_kg_mu = pyro.sample('lam_kg_mu', dist.Gamma(G_a(lam_mu, lam_sd),
                                            G_b(lam_mu, lam_sd)).expand([self.n_dim, self.n_vars, 1]).to_event(3))
        lam_kgi = pyro.sample('lam_kgi', dist.Gamma(G_a(lam_g_mu, lam_g_mu*0.1),
                                            G_b(lam_g_mu, lam_g_mu*0.1)).expand([self.n_dim, self.n_vars, 2]).to_event(3))
        ...
        t_kgON = pyro.sample('t_kgON', dist.Uniform(self.zero, self.one).expand([self.n_dim, 1, self.n_vars, 1]).to_event(3))
        T_kgON = pyro.deterministic('T_kgON', -T_max*self.zero_point_one + t_kgON*T_max*self.one_point_two)
        # Global switch off time for each gene:
        t_kgOFF = pyro.sample('t_kgOFF', dist.Uniform(self.zero, self.one).expand([self.n_dim, 1, self.n_vars, 1]).to_event(3))
        T_kgOFF = pyro.deterministic('T_kgOFF', T_gON + t_gOFF*(T_max*self.one_point_one - T_kgON))
