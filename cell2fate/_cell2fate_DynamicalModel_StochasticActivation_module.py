...
class Cell2fate_DynamicalModel_module(PyroModule):
    ...
    def forward(self, u_data, s_data, idx, batch_index):
        ...
        self.n_states = self.n_modules + 2
        self.n_transitions = self.n_modules
        batch_size = len(idx)
        ...
        # ====================Module Activation based on Markov Chain================ #
        Pi_nn = torch.zeros((self.n_states, self.n_states))
        for n in range(self.n_states):
            if n < self.n_modules:
                Pi_nn[] = pyro.sample('Pi_n' + str(n), dist.Dirichlet(self.n_states - 1).expand([1, self.n_states-1]).to_event(2))
        
        for t in range(self.n_transitions):
            K_ct = pyro.sample('K_c' + str(t), dist.Categorical(Pi_).expand([1, self.n_states-1]).to_event(2))
        ...
        t_cmON = torch.cumsum(torch.concat([self.zero.unsqueeze(0), t_delta[:-1]]), dim = 0).unsqueeze(0).unsqueeze(0)
        T_cmON = pyro.deterministic('T_mON', T_max*t_mON)
