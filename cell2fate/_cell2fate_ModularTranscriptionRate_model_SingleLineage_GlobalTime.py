...
class Cell2fate_ModularTranscriptionRate_model_SingleLineage_GlobalTime(QuantileMixin, PyroSampleMixin, PyroSviTrainMixin, PltExportMixin, BaseModelClass):
    ...
    **model_kwargs
    ...
    def compute_module_summary_statistics(self, adata):
        ...
        for m in range(self.module.model.n_modules):
            mu_m = mu_mRNA_continousAlpha_globalTime_twoStates(
                ...
                torch.tensor(self.samples['post_sample_means']['T_cmON'][...,m]),
                torch.tensor(self.samples['post_sample_means']['T_cmOFF'][...,m]),
                torch.zeros((self.module.model.n_obs, self.module.model.n_vars)))
            adata.obs['Module ' + str(m) + ' Weight'] = torch.sum(torch.sum(mu_m, axis = -1), axis = -1)/inferred_total
        ...
        torch.tensor(self.samples['post_sample_means']['A_mgON'][m,:])/torch.tensor(self.samples['post_sample_means']['beta_g']), axis = 1)
            ...
            adata.obs['Module ' + str(m) + ' State'
             ][self.samples['post_sample_means']['T_c'][:,0,0] > self.samples['post_sample_means']['T_cmON'][:,0,m]
              ] = 'Induction'
            adata.obs['Module ' + str(m) + ' State'
             ][self.samples['post_sample_means']['T_c'][:,0,0] > self.samples['post_sample_means']['T_cmOFF'][:,0,m]
              ] = 'Repression'
            adata.obs['Module ' + str(m) + ' State'
    ...
    def plot_module_summary_statistics(self, adata):
    ...
    def compute_and_plot_module_velocity(self, adata, delete = True, plot = True,
    ...
    def compute_and_plot_total_velocity(self, adata, delete = True, plot = True,
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            ...
            if plot:
                scv.pl.velocity_embedding_stream(adata, basis='umap', save = False, vkey='Velocity',
                                                 **plotting_kwargs, show = True)
            del adata.layers['Spliced Mean']
            if delete:
                del adata.layers['Velocity']

    def compare_module_activation(self, adata, chosen_modules, interval = 0.005, max_time_steps = 1000,
                             ss_fraction = 0.99):
        n_modules = self.module.model.n_modules
        ...
        for m in range(n_modules):
            ...
            ss_spliced = torch.sum(torch.tensor(self.samples['post_sample_means']['A_mgON'][m,:])\
                                   /torch.tensor(self.samples['post_sample_means']['gamma_g']))
            while (fraction < ss_fraction)*(count < max_time_steps)*1 == 1:
                fraction = torch.sum(mu_mRNA_continousAlpha_globalTime_twoStates(
                            torch.tensor(self.samples['post_sample_means']['A_mgON'][m,:]),
                            torch.tensor(0.),
                            torch.tensor(self.samples['post_sample_means']['beta_g']),
                            torch.tensor(self.samples['post_sample_means']['gamma_g']),
                            ...
                            torch.tensor(0.),
                            T_c + 1,
                            torch.zeros((1, self.module.model.n_vars)))[...,1], axis = -1)/ss_spliced
                fraction_list += [fraction.item()]
                T_c = torch.tensor(Tmax*(count+1)*interval).unsqueeze(-1).unsqueeze(-1)
                T_c_list += [T_c.item()]
                count += 1
            if count == max_time_steps:
                print('Could not find steady state. Increase interval or max_time_steps or decrease ss_fraction.')
            ax[1].scatter(T_c_list, fraction_list, s = 10, label = 'Module ' + str(m))
            ...
            abundance = torch.sum(mu_mRNA_continousAlpha_globalTime_twoStates(
                    ...
                    torch.tensor(self.samples['post_sample_means']['lam_mi'][m,:]),
                    torch.tensor(self.samples['post_sample_means']['T_c'][...,0]),
                    torch.tensor(np.mean(self.samples['post_sample_means']['T_cmON'][...,m])),
                    torch.tensor(np.mean(self.samples['post_sample_means']['T_cmOFF'][...,m])),
                    torch.zeros((self.module.model.n_obs, self.module.model.n_vars)))[...,1], axis = -1)
            ax[0].scatter(self.samples['post_sample_means']['T_c'].flatten() - np.min(self.samples['post_sample_means']['T_c'].flatten()),
            ...
            T_min = np.min(self.samples['post_sample_means']['T_c'].flatten())
            T_max = np.max(self.samples['post_sample_means']['T_c'].flatten()) - T_min
            T_ON = self.samples['post_sample_means']['T_cmON'][...,m].flatten() - T_min
            T_OFF = self.samples['post_sample_means']['T_cmOFF'][...,m].flatten() - T_min
            ax[2].scatter(T_ON, T_OFF, alpha = 0.05, label = 'Module ' + str(m))
            ax[2].set_ylabel('T_cmOFF')
            ax[2].set_xlabel('T_cmON')
            ax[2].set_title('Module Switch Times')
            leg = ax[2].legend(frameon=False)
            for lh in leg.legendHandles: 
                lh.set_alpha(1)

    def plot_technical_variables(self, adata):
        ...
        ax[2,0].hist(self.samples['post_sample_means']['detection_y_gi'][...,1].flatten(),
                     bins = 100, alpha = 0.75, label = 'spliced', color = 'blue')
        ax[2,0].set_xlabel('Relative Detection Efficiency across genes')
        ax[2,0].set_ylabel('Number of Genes')
        ax[2,0].set_title('log10 Overdispersion factor across genes \n (smaller = more variance)')
    ...
    def get_module_top_features(self, adata, species = 'Mouse', p_adj_cutoff = 0.01):
        ...
        if species == 'Human':
            TFs = np.array(pd.read_csv('/nfs/team283/aa16/cell2fate/cell2fate/Human_TFs.txt', header=None, index_col=False)).flatten()
        elif species == 'Mouse':
            TFs = np.array(pd.read_csv('/nfs/team283/aa16/cell2fate/cell2fate/Mouse_TFs.txt', header=None, index_col=False)).flatten()
