...
class Cell2fate_DynamicalModel_module(PyroModule):
    ...
    def forward(self, u_data, s_data, idx, batch_index):
        ...
        beta_g = pyro.sample('beta_g', dist.Gamma(splicing_alpha, splicing_alpha/splicing_mean).expand([self.n_modules, 1,self.n_vars]).to_event(3))
        # Degredation rate:
        ...
        gamma_g = pyro.sample('gamma_g', dist.Gamma(degredation_alpha, degredation_alpha/degredation_mean).expand([self.n_modules, 1,self.n_vars]).to_event(3))
...
#         t_mON = pyro.sample('t_mON', dist.Uniform(self.zero, self.one).expand([1, 1, self.n_modules]).to_event(2))
        ...
        for m in range(self.n_modules):
            mu_total += mu_mRNA_continousAlpha_globalTime_twoStates(
                A_mgON[m,:], A_mgOFF, beta_g[m,...], gamma_g[m,...], lam_mi[m,...], T_c[:,:,0], T_mON[:,:,m], T_mOFF[:,:,m], self.zeros[idx,...])
