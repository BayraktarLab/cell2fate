...
class Cell2fate_DynamicalModel_PriorKnowledge_Sequential_module(PyroModule):
    ...
    def __init__(
        ...
        self.register_buffer("ones_lam", torch.ones(self.n_modules, 1, 2))
        self.register_buffer("zeros_gg", torch.zeros(self.n_vars, self.n_vars))
        self.register_buffer("zeros_g", torch.zeros(1, self.n_vars))
    ...
    def forward(self, u_data, s_data, idx, batch_index):
        ...
        # Global switch on time for each cell:
        ...
        mu_total_T0_list = [self.zeros_g]
        mu_total_T0_list += [mu_total_T0_list[-1] + g_0g]
        A_mgON_list = [g_0g*gamma_g]
        g_fg_list = [g_0g]
        for m in range(1, self.n_modules):
            mean = torch.einsum('it, gt->ig', mu_total_T0_list[-1][:,self.is_tf], X_gt) + 10**(-5)
            alpha = 1000
            ...
            mu_total_T0_list += [mu_total_T0_list[-1] + mu_mRNA_continousAlpha_globalTime_twoStates(
                A_mgON_list[-1], A_mgOFF, beta_g, gamma_g, lam_mi[m,...],
                T_mON[:,:,m], T_mON[:,:,m-1], T_mOFF[:,:,m-1], self.zeros_g)[...,1]]
            mu_total += mu_mRNA_continousAlpha_globalTime_twoStates(
                A_mgON_list[-1], A_mgOFF, beta_g, gamma_g, lam_mi[m,...], T_c[:,:,0], T_mON[:,:,m], T_mOFF[:,:,m], self.zeros[idx,...])     
        with obs_plate:
